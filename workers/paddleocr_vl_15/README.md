# PaddleOCR-VL 1.5 로컬 worker

이 디렉터리는 `PaddleOCR-VL-1.5`와 `PP-DocLayoutV3`를 Hugging Face에서
...
1.6 worker와 같은 PaddleOCR/PaddleX 런타임 및 레이아웃 모델을 사용하므로
두 결과의 차이는 VLM 체크포인트 차이로 좁혀집니다.
...
cd D:\path\to\pptx-wiki\workers\paddleocr_vl_15
...
루트의 `models\paddleocr_vl_15`에 보관됩니다. 모델 위치를 바꾸려면 설치 시
`-ModelDir "D:\models\paddleocr_vl_15"`을 지정하고 root `config.yml`의
...
```powershell
.\.venv\Scripts\python.exe .\download.py --model-dir "..\..\models\paddleocr_vl_15"
```

모델 리비전은 모델 디렉터리의 `manifest.json`에 기록되며 worker 시작 시
필수 파일과 함께 다시 검사됩니다. 동일 GPU에서 1.5와 1.6 worker를 동시에 띄우면 메모리
부족이 날 수 있으므로 비교할 때는 한 worker를 종료한 뒤 다른 worker를
시작하는 방식을 권장합니다.
